...
- 백엔드 테스트 116개, 프론트 vitest 24개, 타입체크·린트, 실험 산출물 정합성 검사기. push마다 CI가 돌고 건너뛰는 검사가 없다.
...
- **배포 설정이 없다.** Dockerfile이 없고 로컬에서만 돈다. (CI는 GitHub Actions로 백엔드·프론트 검사만 돌린다 — 실험은 GPU가 필요해 빠져 있다.)
- **DB가 없다.** 결과는 파일로 쌓인다.
...
정밀도가 평균 +0.42 올랐고, 맞는 자에서도 손해가 없었다.
