...
현재 작업 순서와 완료 조건은 [25번 고도화 계획](docs/25-advancement-roadmap.md), 데스크탑 인수인계는 [CLAUDE.md](CLAUDE.md), 성능 수치의 조건과 한계는 [현재 근거표](docs/current-evidence.md)를 따른다.

## 이게 뭔가

**AIDA (AI Data Assurance)** — 객체탐지 AI 학습데이터의 바운딩박스 라벨 오류를
진단하고 재검수 우선순위를 제공하는 연구 프로토타입. 품질 인증이나 실제 고객의 검수 효율 개선은 아직 검증하지 않았다.

객체탐지 모델은 사람이 그린 바운딩박스 라벨을 정답으로 학습한다. 그런데 라벨에
가로/세로 크기나 회전각 오류가 섞여 있으면 모델 성능이 떨어질 수 있다.
모델 성능만으로 모델 문제와 라벨 오류를 구분하기는 어렵다. AIDA는
...
- 백엔드·프론트 순수 로직·GPU 없는 실험 로직 검사와 타입체크·린트·빌드를 CI에 구성했다. 실제 추론과 화면 비동기 동작을 포함한 검사 경계는 [testing-boundary](docs/testing-boundary.md)를 참고한다.
- **검수를 키보드로 한다.** `j`/`k`로 이동, `f` 오류, `d` 아님. 판정하면
  다음 줄로 넘어간다. 지난 진단은 새로고침 뒤에도 다시 열 수 있다.

### 안 되는 것

- **첫 예비 비교에서 제품 순서가 단순한 방법보다 나빴다.** 핵심 질문은 "같은
  검수량에서 단순 IoU 불일치 순서보다 실제 오류를 더 찾는가"다. KITTI Car 300장을
  판정자 1인이 가림 판정한 탐색 비교(prelim1)에서, 기존 라벨 후보 상위 90건 중
  판정자가 오류로 본 라벨이 **AIDA 제품 순서 4건, 단순 IoU 순서 14건**이었다
  ([결과](docs/prelim1-results.md)). 그래서 후보 순위를 버전으로 나눴고
  ([ADR](docs/adr-ranking-separation.md)), 새 순서(v2)는 아직 평가하지 않았다.
  **후보 생성과 누락 탐지의 가치도 아직 재지 않았다.** 위 정밀도 수치는 주입 오류
  기준이라 이 질문의 답이 아니다.
...
10% 정밀도가 COCO 평가에서 자기 도메인 모델 82.4% 대비 KITTI 모델 26.0%로 떨어졌다.** 이 프로젝트가 지금 답을 갖고
있지 않은 문제다.

다만 **피해를 줄이는 법은 하나 찾았다**(docs/21 AN). 자가 어긋나면 정밀도가
떨어지기만 하는 게 아니라 **재검수 목록의 순서가 뒤집힌다** — 맨 위가 아래보다
나빴다. 계통적이라고 판정한 오류 유형을 먼저 보여주게 고치니 상위 5건의
정밀도가 **평균 +0.41**(시드 3개, +0.385~+0.422) 올랐고, 맞는 자에서도 손해가
없었다.
...
API로 제공하고, `frontend/`가 이를 시각화한다. 이 실험 결과 조회 흐름 외에,
업로드 진단은 백엔드가 추론 서브프로세스를 호출하고 결과 JSON을 읽는다.
검수 판정도 별도로 저장한다. 아래 그림은 실험 결과 조회 흐름을 나타낸다.
