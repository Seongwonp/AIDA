...
# (%p, docs/21 AF, 조건 30개). 평균만 보면 안 보이는 값이다.
#
# 단일 클래스 자가 다중 클래스 자보다 2배 안정적이다(±2.50 vs ±5.27).
# 아는 것만 말하고 나머지는 침묵하는 자는 학습 시드가 바뀌어도 흔들릴
# 경계가 적기 때문이다. 고객마다 기준 모델을 새로 학습하는 구조에서는
# 평균보다 이쪽이 중요할 수 있다.
#
# 시드 3개로 쟀을 때는 ±0.46 vs ±4.80(열 배)이었는데, 7개로 늘리니 2배로
# 줄었다. 세 값이 우연히 가까웠던 것이다 — AD에 그럴 수 있다고 적어둔
# 한계가 실제로 현실이 됐다. n이 작을 때의 표준편차는 그 자체가 흔들린다.
RULER_SEED_SPREAD_PP = {1: 2.50, 4: 5.27}
DEFAULT_SEED_SPREAD_PP = 5.27
