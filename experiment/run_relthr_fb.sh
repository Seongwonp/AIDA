...
#   PER_COND=1  조건별 기록까지 남긴다 (어느 조건이 손해인가)
set -e
cd "$(dirname "$0")"
for s in ${COCO_SEEDS:-42 123 2024 2025 31337 7 777}; do
  AIDA_DATASET=coco ./venv/Scripts/python.exe relative_threshold.py --seed $s --mode fallback ${PER_COND:+--per-condition} \
...
for s in ${KITTI_SEEDS:-42 123 2024 2025 31337 7 777}; do
  AIDA_CLASSES="Car,Van,Pedestrian,Cyclist" AIDA_FRAME_SELECT=cyclist_rich \
    ./venv/Scripts/python.exe relative_threshold.py --seed $s --mode fallback ${PER_COND:+--per-condition} \
