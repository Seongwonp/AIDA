        ...
        <div className="upload-result">
          ...
          {labelResult.order_basis === "relative" && (
            <p className="report-caveat">
              ...
              잘 맞지 않을 때 나타나고, 그때는 이 순서가 크게 도움이 됩니다
              (실측: docs/21 AO).
              {labelResult.order_risk && (
                <>
                  {" "}
                  <strong>다만 이 데이터에서는 먼저 볼 유형으로 크기·위치 계열만
                  올라왔습니다.</strong> 라벨 누락·중복·클래스 오기입이 적게 섞여 있으면
                  모델 자신의 오탐이 진짜 오류보다 많아져 이런 모습이 되고, 그때는 이
                  순서가 오탐을 좇습니다. 실측에서 이 모습일 때 약 절반이 그랬습니다
                  (반대로 누락·중복·오기입이 올라온 경우는 150번 중 한 번도 나빠지지
                  않았습니다 — docs/21 BB). 크기·위치 오류가 실제로 많은 데이터라면
                  해당하지 않습니다.
                </>
              )}
            </p>
