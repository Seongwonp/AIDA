...
| **car_v1_e100** (100에폭, best=93) | **71.6%** | 51% | 78% | 86% | 96% | 233 | 566 (누락 21) |
| COCO Car | 49.0% | 22% | 51% | 72% | 80% | 505 | 466 (누락 26) |
| KITTI Car | 30.6% | 3% | 30% | 57% | 56% | 935 | 219 (누락 7) |

- **100에폭 재학습(사용자 승인, 4,093초 ≈ 68분)** — `nuimages_fit_check_car_v1_e100.json`. ruler_val fitness
  0.5039(50에폭) → **0.5162**(93에폭이 best), mAP50-95 0.479 → 0.492, R 0.604 → 0.625, P 0.866 → 0.835.
  fit_check 짚은 비율 69.9% → 71.6%. **자 선택 규칙은 ruler_val fitness로 한다는 조언을 따를지, 어느 자를 쓸지는
  사용자가 사전 등록에서 정한다.**
- 카메라별(50에폭) 64.5%(BACK_RIGHT)~77.6%(FRONT_RIGHT). 지역 차이는 기록 7개라 해석하지 않는다.
