# 14. 대시보드 고도화 계획 — 상세 통계 표 + CSV 다운로드

**상태: 계획만 확정, 실행은 대기 중.** CUDA 데스크탑에서 13개 조건 전체를 동일
50epoch로 재실행하는 실험이 끝나면([09-getting-started.md](./09-getting-started.md)
참고) 이 계획대로 진행한다. 지금(실험 진행 중)은 코드를 건드리지 않는다 —
`metrics.csv`가 계속 갱신되는 중이라 지금 만들면 화면을 두 번 확인해야 한다.
