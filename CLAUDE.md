...
5. 지금 고를 수 있는 것은 세 가지뿐이다 — **A** 데이터셋당 관측 범위 운영안
   ...
   **A를 "전체 계획이 비외삽"이라고 부르지 않는다.** 관측 범위인 것은
   *데이터셋당 1블록*뿐이고(`per_dataset_evidence=observed`), `D>1`이면
   데이터셋 간 난이도 차이와 블록 반복은 여전히 `evidence=extrapolated`다.
...
최근 통과 수는 experiment 405 · backend 252 · frontend 176이다. experiment 405는
2026-09-13 노트북에서 CI와 같은 목록(`experiment/requirements-ci.txt`)만 깐
가상환경으로 직접 돌린 결과이고, backend·frontend 수는 같은 날 데스크탑 기록이다.
